...
function downloadPDF(data,title){
  ...
  const win=window.open(url,'_blank');if(win){win.onload=()=>{win.print();URL.revokeObjectURL(url)}}
}

function formatExport(logs,type){
  return logs.map(l=>type==='staff'?{'Badge':l.badgeNumber||'','Name':l.name||'','Role':l.role||'','Sign In':l.signIn?`${formatShortDate(l.signIn)} ${formatTime(l.signIn)}`:'—','Sign Out':l.signOut?`${formatShortDate(l.signOut)} ${formatTime(l.signOut)}`:'—'}:{'Name':l.name||'','Phone':l.phone||'','Visiting':l.host||'','Reason':l.reason||'','Sign In':l.signIn?`${formatShortDate(l.signIn)} ${formatTime(l.signIn)}`:'—','Sign Out':l.signOut?`${formatShortDate(l.signOut)} ${formatTime(l.signOut)}`:'—'})
}

function filterLogs(logs,q,d,type){
  const query=q.toLowerCase().trim()
  return logs.filter(l=>{
    const t=!query||(l.name?.toLowerCase().includes(query)||(type==='staff'&&l.badgeNumber?.toLowerCase().includes(query))||(type==='visitor'&&l.phone?.includes(query))||l.role?.toLowerCase().includes(query)||l.host?.toLowerCase().includes(query))
    const dt=!d||(l.signIn&&new Date(l.signIn).toISOString().slice(0,10)===d)
    return t&&dt
  })
}

function todayOnly(logs){const s=new Date();s.setHours(0,0,0,0);return logs.filter(l=>l.signIn&&new Date(l.signIn)>=s)}
...
export default function AdminDashboard() {
  ...
  useEffect(()=>{
    ...
    Promise.all([getStaffLogs(),getVisitorLogs()]).then(([s,v])=>{setStaffLogs(s.data);setVisitorLogs(v.data)}).finally(()=>setLoading(false))
  },[])
  ...
  const exportTitle=tab==='staff'?'Staff & Volunteer Attendance Log':'Visitor Log'
  const todayS=todayOnly(staffLogs); const todayV=todayOnly(visitorLogs)

  return (
    <div className={styles.wrapper}>
      <div className={styles.topBar}>
        <div><div className={styles.topTitle}>Admin Dashboard</div><div className={styles.topSub}>Welcome, {admin?.name||'Administrator'}</div></div>
        <button className={styles.logoutBtn} onClick={handleLogout}>
          <svg fill="none" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15m3 0l3-3m0 0l-3-3m3 3H9"/></svg>Logout
        </button>
      </div>
      ...
      <div className={styles.tableCard}>
        <div className={styles.tableToolbar}>
          ...
          <div className={styles.downloadBtns}>
            <button className={styles.dlBtn} onClick={()=>downloadCSV(exportData,`${exportTitle}.csv`)} disabled={!filtered.length}><svg fill="none" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3"/></svg>CSV</button>
            <button className={styles.dlBtn} onClick={()=>downloadPDF(exportData,exportTitle)} disabled={!filtered.length}><svg fill="none" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z"/></svg>PDF</button>
          </div>
        </div>
        <div className={styles.searchBar}>
          <div className={styles.searchWrap}>
            ...
            <input className={styles.searchInput} type="text" placeholder={tab==='staff'?'Search by name, badge or role…':'Search by name, phone or host…'} value={search} onChange={e=>setSearch(e.target.value)}/>
        ...
        {(search||dateFilter)&&<div className={styles.resultCount}>{filtered.length} result{filtered.length!==1?'s':''} found{dateFilter&&` for ${new Date(dateFilter+'T00:00:00').toLocaleDateString('en-GB',{day:'numeric',month:'long',year:'numeric'})}`}</div>}
        {loading?<div className={styles.empty}>Loading...</div>:filtered.length===0?<div className={styles.empty}>{search||dateFilter?'No records match your search.':`No ${tab} activity recorded yet.`}</div>:(
            ...
            {tab==='staff'?(
              <table className={styles.table}>
                ...
                <tbody>{filtered.map((l,i)=>(
                  <tr key={i}>
                    <td className={styles.nameCell}>{l.name}</td><td className={styles.mono}>{l.badgeNumber}</td><td className={styles.muted}>{l.role}</td>
              ...
              <table className={styles.table}>
                ...
                <tbody>{filtered.map((l,i)=>(
                  <tr key={i}>
                    <td className={styles.nameCell}>{l.name}</td><td className={styles.muted}>{l.phone}</td><td className={styles.muted}>{l.host}</td><td className={styles.muted}>{l.reason}</td>
                    <td className={styles.muted}>{l.signIn?formatShortDate(l.signIn):'—'}</td>
                    <td className={styles.timeIn}>{l.signIn?formatTime(l.signIn):'—'}</td>
                    <td className={l.signOut?styles.timeOut:styles.timePending}>{l.signOut?formatTime(l.signOut):'Active'}</td>
                    <td><span className={`${styles.pill} ${l.signOut?styles.pillOut:styles.pillIn}`}>{l.signOut?'Signed Out':'In Building'}</span></td>
